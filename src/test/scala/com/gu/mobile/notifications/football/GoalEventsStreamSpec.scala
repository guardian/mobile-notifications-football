...
class GoalEventsStreamSpec extends WordSpec with Matchers with GoalEventStream {
  import MatchDaysTestData._
  ...
  "Streams.goalEvents" when {
    "given a stream of two MatchDays over which a goal has been scored" should {
      "produce a stream containing an event for the goal that was scored" in {
        val goalEvents = getGoalEvents(matchDays).toBlockingObservable.toList
